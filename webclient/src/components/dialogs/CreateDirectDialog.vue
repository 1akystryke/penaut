  ...
  >
    <div class="dialog-card">
      <div class="dialog-header">
        ...
        <div>
          <h3 class="dialog-title">Новая переписка</h3>
          <p class="dialog-sub">Выберите пользователя для личного чата</p>
        </div>
      </div>
      ...
      <div class="dialog-footer">
        ...
        <button class="btn-primary" :disabled="!selectedUserId" @click="submit">Начать чат</button>
      </div>
    </div>
  </v-dialog>
</template>
...
defineProps({ modelValue: Boolean, users: Array })
const emit = defineEmits(['update:modelValue', 'create'])

const selectedUserId = ref('')

function submit() {
  if (selectedUserId.value) emit('create', selectedUserId.value)
}
</script>
